...
def create_layout():
    layout = html.Div(id='container-grafico-abc-fornecedores', children=[
        dbc.Row([
            ...
            dbc.Col(
                dbc.Card([
                    ...
                    dbc.Row([
                        dbc.Col([
                            dcc.Graph(
                                id='bar-chart-total-abc-fornecedor',
...
def update_chart(start_date, end_date, selected_rep_ids, selected_emp_ids, selected_tabpreco_id, selected_cfgpedido_id):
    ...
    SELECT
        CONTAMOV.NUMEROCM,
        CONTAMOV.NOME AS FORNECEDOR,
        COALESCE(SUM(NFITEM.VALORTOTAL), 0) AS Total_Compras,
        COUNT(NFCAB.NOTA) AS QUNTIDADE_NOTA,
    ...
    WHERE
        ...
        {rep_ids_clause}    
    GROUP BY
        CONTAMOV.NUMEROCM, CONTAMOV.NOME
    ORDER BY
        Total_Compras DESC
    """
    ...
    dados_grafico = [
        {
            'x': df['FORNECEDOR'],  # Categorias no eixo X
            'y': df['TOTAL_COMPRAS'],  # Valores no eixo Y
            'type': 'bar',
            'name': 'Total Compras',
            'text': [f'Fornecedor: {fornecedor}<br>Quantidade: {locale.format_string("%d", quant, grouping=True)}<br>Valor: R$ {locale.format_string("%0.2f", valor, grouping=True)}' 
                    for fornecedor, quant, valor in zip(df['FORNECEDOR'], df['QUNTIDADE_NOTA'], df['TOTAL_COMPRAS'])],
            'hoverinfo': 'text'
        },
        {
            'x': df['FORNECEDOR'],
            ...
            'yaxis': 'y2',
            'text': [f'Fornecedor: {fornecedor}<br>Porcentagem de Compra: {locale.format_string("%.2f", porcentagem)}%' 
                    for fornecedor, porcentagem in zip(df['FORNECEDOR'], df['PORCENTAGEM_COMPRA'])],
            'hoverinfo': 'text'
        }
    ]

    fig_layout = {
        'title': 'Análise ABC de Compras por Fornecedor',
        'xaxis': {'title': 'Fornecedor', 'tickangle': -45},
        'yaxis': {
