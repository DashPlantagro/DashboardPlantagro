...
def buscar_total_transacoes_periodo(start_date=None, end_date=None, selected_reps_ids=None,selected_emp_ids=None,selected_tabpreco_id=None, selected_cfgpedido_id=None):
    ...
    query = f"""
            SELECT 
                count(NFCAB.SEQNOTA) AS total_vendas
            FROM
                NFCAB
            JOIN
    ...
    return df['TOTAL_VENDAS'].iloc[0] if not df.empty else 0    

def create_layout():
    total_clientes = buscar_total_transacoes_periodo()  # Busca o total de clientes
    layout = html.Div(id='container-total-transacao-por-periodo', children=[
        dbc.Row([
            dbc.Col(width=1),
            dbc.Col(
                dbc.Card([
                    html.H2('Total de Transações por Perido', style={"font-family": "Voltaire", "font-size": "20px", "text-align": "center"}),
                ...
                dbc.Row([
                    dbc.Col([
                        dcc.Graph(
                            id='bar-chart-transacoes-vendas-periodo',
